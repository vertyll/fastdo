...
export default registerAs(
  ...
  (): AppConfig => ({
    ...
    database: {
      ...
      retryAttempts: Number.parseInt(process.env.DATABASE_RETRY_ATTEMPTS || '10', 10),
      retryDelay: Number.parseInt(process.env.DATABASE_RETRY_DELAY || '3000', 10),
    ...
    security: {
      ...
      cookie: {
        refreshToken: {
          ...
          maxAge: Number.parseInt(process.env.COOKIE_MAX_AGE || String(7 * 24 * 60 * 60 * 1000), 10),
      ...
      helmet: {
        crossOriginResourcePolicy:
          (process.env.HELMET_CROSS_ORIGIN_RESOURCE_POLICY as
            | 'same-origin'
            | 'same-site'
            | 'cross-origin'
            | undefined) || 'cross-origin',
    ...
    file: {
      ...
      validation: {
        maxSize: Number.parseInt(process.env.MAX_FILE_SIZE || '') || FILE_CONSTANTS.MAX_FILE_SIZE,
