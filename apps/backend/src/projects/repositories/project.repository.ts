...
export class ProjectRepository extends Repository<Project> {
  ...
  public async findOneWithDetails(id: number, userId: number, currentLanguage: string = 'pl'): Promise<Project> {
    ...
    if (project.type) {
      const typeTranslation = project.type.translations?.find(t => t.language.code === currentLanguage);
      project.type = {
        ...project.type,
        name: typeTranslation?.name || project.type.translations?.[0]?.name || '',
        description: typeTranslation?.description || project.type.translations?.[0]?.description,
        translations: undefined, // Remove translations to clean up response
      } as any;
    }
